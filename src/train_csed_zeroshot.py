...
def evaluations(args, model, scaler, inner_loop_optimizer, eval_iter):
    ...
    with torch.autocast(device_type=args.device, dtype=torch.float16): 
        for task in range(len(query_set)):
            ...
            fast_weights = remove_base_model_from_keys(OrderedDict(model.named_parameters()))
            ...
            with torch.no_grad():
                ...
                for query in batches:
                    ...
                    features.extend(outputs[2].cpu().numpy().tolist())  # use trigger_aware_logits
        ...
        for task in range(len(query_set)):
            with torch.no_grad():
                ...
                for query in batches:
                    ...
                    trigger_logits, event_logits = None, None
                    input_ids, attention_mask, labels = data['input_ids'], data['attention_mask'], data['label']
                    outputs = functional_modifiedroberta(OrderedDict(model.named_parameters()),prev_fast_weights, model.config,
                                                         input_ids=input_ids,
                                                         attention_mask=attention_mask,
                                                         is_train=False)
                    gts.extend(labels.cpu().numpy().tolist())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--K', default=10, type=int, help='number of support instances per class (i.e., shot)')  # not used
